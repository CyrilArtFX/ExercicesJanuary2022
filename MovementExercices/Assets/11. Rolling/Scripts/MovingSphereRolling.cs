...
public class MovingSphereRolling : MonoBehaviour
{
    [SerializeField] Transform playerInputSpace = default, ball = default;

    [SerializeField, Range(0f, 100f)] float maxSpeed = 10f, maxClimbSpeed = 2f, maxSwimSpeed = 5f;
    [SerializeField, Range(0f, 100f)] float maxAcceleration = 10f, maxAirAcceleration = 1f, maxClimbAcceleration = 20f, maxSwimAcceleration = 5f;
    ...
    [SerializeField, Min(0.00001f)] float ballRadius = 0.5f;
    ...
    void FixedUpdate()
    {
        Vector3 gravity = CustomGravityComplexGravity.GetGravity(body.position, out upAxis);
    ...
    void UpdateState()
    {
        ...
        if (CheckClimbing() || CheckSwimming() || OnGround || SnapToGround() || CheckSteepContact())
        {
    ...
    void Jump(Vector3 gravity)
    {
        ...
        if (InWater) jumpSpeed *= Mathf.Max(0f, 1f - submergence / swimTreshold);
